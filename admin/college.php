		...
		<header id="header">
			...
			<div class="container">
				<div class="logo float-left">
					...
					<h1 class="text-light"><a href="college.php" class="scrollto"><span>Web•IMS</span></a></h1>
				...
				<nav class="main-nav float-right d-none d-lg-block">
					<ul>
						...
						<ul>
						  ...
						  <li class="active"><a href="college.php">College</a></li>
				...
				<div class="container">
					<div class="row">
						...
						<div class="about-content"  style="text-align:justify;">
							<h2>Colleges</h2>
							<h4>Modifies the details of each department/colleges</h4>
						</div>
				...
				<div class="modal-content"> 
					<form method="post" id="insert_form">
						...
						<div class="modal-body">  
							<label><strong>College Short Name </strong> <font color="red" style="font-weight: bold;">*</font> </label>  
							<input type="text" name="cshortname" id="cshortname" class="form-control" size="25" required />  
							<br />  
							
							<label><strong>College Full Name </strong> <font color="red" style="font-weight: bold;">*</font> </label>  
	...
	$(document).ready(function(){
		...
		function fetch_data(){
			var dataTable = $('#user_data').DataTable({
				...
				"order" : [],
				"ajax" : {
					url:"../components/config-college/fetch.php",
					type:"POST"
				}
			});
		}
		...
		$(document).on('click', '.edit', function(){  
			...
			$.ajax({  
                url:"../components/config-college/write.php",  
                method:"POST",  
                data:{college_id:college_id},  
                dataType:"json",  
                success:function(data){  
                    $('#cshortname').val(data.college_shortname);  
                    $('#cfullname').val(data.college_fullname); 
		...
		$('#insert_form').on("submit", function(event){  
			...
			if($('#cshortname').val() == ""){  
				...
				$.ajax({
					url : "../components/config-college/check.php",
					type : "POST",
					cache:false,
					data : {cshortname:cshortname, college_id:college_id},
					success:function(result){
						if (result == 1) {
							alert("College alias is already taken!");  
						}
						else{
							$.ajax({  
								url:"../components/config-college/insert.php",  
								method:"POST",  
		...
		$(document).on('click', '.delete', function(){
			...
			if(confirm("Are you sure you want to remove this?")){
				$.ajax({
					url:"../components/config-college/delete.php",
					method:"POST",
					data:{id:id},
					success:function(data){
		...
		$(document).on('click', '.archive', function(){
			...
			if(confirm("Are you sure you want to archive this?")){
				$.ajax({
					url:"../components/config-college/archive.php",
					method:"POST",
					data:{id:id},
					success:function(data){
						$('#alert_message').html('<div class="alert alert-success">'+data+'</div>');
		...
		$(document).on('click', '.restore', function(){
			...
			if(confirm("Are you sure you want to restore this?")){
				$.ajax({
					url:"../components/config-college/restore.php",
					method:"POST",
					data:{id:id},
					success:function(data){
						$('#alert_message').html('<div class="alert alert-success">'+data+'</div>');
