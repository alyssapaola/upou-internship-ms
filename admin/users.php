<?php
	...
	*/
	...
	$role_users = '';
	$query = "SELECT * FROM tbl_role WHERE tbl_role.role_name LIKE '%staff%' OR tbl_role.role_name LIKE '%manager%'";
	$statement = $connect->prepare($query);
	$statement->execute();
	$result = $statement->fetchAll();
	foreach($result as $row){
		$role_users .= '<option value="'.$row['role_id'].'">'.$row['role_name'].'</option>';
	}
?>
		...
		<header id="header">
			...
			<div class="container">
				<div class="logo float-left">
					...
					<h1 class="text-light"><a href="users.php" class="scrollto"><span>Web•IMS</span></a></h1>
				...
				<div class="container">
					<div class="row">
						...
						<div class="about-content"  style="text-align:justify;">
							<h2>Users</h2>
							<h4>Displays all users of the system - students, administrators, and industry partners</h4>
						</div>
				...
				<div class="modal-content">  
					<form method="post" id="insert_form">
						...
						<div class="modal-body">  
							...
							<label><strong>User Type</strong> <font color="red" style="font-weight: bold;">*</font> </label>
							<select name="accttype"  id="accttype" class="form-control" required>
								<option value="" disabled selected>Choose here</option>
								<?php $acctype_qry = "SELECT * FROM tbl_role WHERE tbl_role.role_name NOT LIKE '%student%' AND tbl_role.role_name NOT LIKE '%employer%' ";
	...
	$(document).ready(function(){
		...
		function fetch_data(filter_role = ''){
			var dataTable = $('#user_data').DataTable({
				...
				"searching" : true,
				"ajax" : {
					url:"../components/config-users/fetch.php",
					type:"POST",
					data:{
		...
		$(document).on('click', '.edit', function(){  
			...
			$.ajax({  
                url:"../components/config-users/write.php",  
                method:"POST",  
				...
				$.ajax({
					...
					success:function(result){
						if (result == 1) {
							...
							$.ajax({  
								url:"../components/config-users/insert.php",  
								method:"POST",  
		...
		$(document).on('click', '.view', function(){  
           ...
           if(user_id != ''){  
                $.ajax({  
                    url:"../components/config-users/select.php",  
                    method:"POST",  
                    data:{user_id:user_id},  
                    success:function(data){  
