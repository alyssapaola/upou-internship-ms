<?php
	...
	$columns = array('template_id','template_name', 'template_desc', 'temp_category_name', 'template_id');
	...
	$query = "SELECT * FROM tbl_template 
			...
			WHERE tbl_template.delete_flag = '0' ";
	...
	while($row = mysqli_fetch_array($result)){
		...
		$sub_array[] = '<button type="button" name="view"  class="view '.$flag.'"  id="'.$row["template_id"].'">View</button>
						<button type="button" name="edit"  class="edit '.$flag.'"  id="'.$row["template_id"].'">Edit</button>
						<button type="button name="'.$name.'" class="'.$name." ".$flag.'" id="'.$row["template_id"].'">'.$value.'</button>
						<button type="button" name="delete" class="delete '.$flag.'" id="'.$row["template_id"].'">Delete</button>';
