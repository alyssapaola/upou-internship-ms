<?php
	...
	*/
	...
	if(!empty($_POST["register"])) {
		require_once "../components/config-listing/edit.php";
	}
	
	$query = $_SESSION['query'];
	$result = mysqli_query($con, $query);
	while($row = mysqli_fetch_array($result)) {  
		$job_id_db = $row["job_id"];
		
		$job_name_db = $row["job_name"];
		$job_desc_db = $row["job_desc"];
		$num_vacancy_db = $row["num_vacancy"];
		$allowance_flag_db = $row["allowance_flag"];
		
		if ($allowance_flag_db == "1"){
			$checked = "checked";
		}
	}
?>
				...
				<div class="form">
					...
					<form method="post" action="" role="form" class="contactForm" enctype="multipart/form-data" onsubmit="return validateContactForm()">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control" name="job_title" id="job_title" value="<?php echo $job_name_db; ?>" />
							
							<input type="hidden" name="job_id" id="job_id" class="form-control" size="25" value="<?php echo $job_id_db; ?>" >
							<div id="job_title-info" class="info"></div>
						</div>
						
						<div class="form-group">
							<label for="job_desc"><b>Description <font color="red">*</font> </b></label>
							<textarea name="job_desc" id="job_desc" class="form-control" ><?php echo $job_desc_db; ?></textarea>
							<div id="job_desc-info" class="info"></div>
						</div>		
						
						<div class="form-group">
							<label for="job_vac"><b>No. of Vacancies <font color="red">*</font> </b></label>
							<input type="text" class="form-control" name="job_vac" id="job_vac" value="<?php echo $num_vacancy_db; ?>" />
										...
										while($college_row = mysqli_fetch_assoc($college_rslt)){
											...
											if(mysqli_num_rows($course_rslt) > 0){
												while($course_row = mysqli_fetch_assoc($course_rslt)){
													...
													$queryCheck = "SELECT course_id FROM  tbl_job_category 
															WHERE course_id = '$course_id' AND job_id = '$job_id_db' AND delete_flag = '0'";  
													$resultCheck = mysqli_query($con, $queryCheck);
													if(mysqli_num_rows($resultCheck) > 0){
														echo "<option value='$course_id' selected >".$course_fullname."</option>";
													}
													else{
														echo "<option value='$course_id' >".$course_fullname."</option>";
													}
						...
						<div class="form-group">
							<input type="checkbox" name="job_salary" id="job_salary" <?php echo $checked; ?> >
