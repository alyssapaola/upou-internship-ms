		...
		<header id="header">
			...
			<div class="container">
				<div class="logo float-left">
					...
					<h1 class="text-light"><a href="intp-hours.php" class="scrollto"><span>Web•IMS</span></a></h1>
						...
						<li class="active drop-down"><a href="#">Internship Portal</a>
							<ul>
								...
								<li class="active drop-down"><a href="#">Internship/Credit Hours</a>
									<ul>
										<li class="active"><a href="intp-hours.php">Internship Hours</a></li>
				...
				<div class="container">
					<div class="row">
						...
						<div class="about-content"  style="text-align:justify;">
							...
							<h4>Modifies the internship hours record per courses based on curriculum</h4>
						</div>
					...
					<form method="post" id="insert_form">
						...
						<div class="modal-body">  
							...
							<select name="college_type"  id="college_type" class="form-control" required>
								...
								<?php
									$college_type_qry = "SELECT * FROM tbl_college WHERE delete_flag='0' ORDER BY college_fullname ASC";
							...
							<select name="course_type"  id="course_type" class="form-control" required>
								<option value="" disabled selected >Choose here</option>
								<?php
									$course_type_qry = "SELECT * FROM tbl_course WHERE delete_flag='0' ORDER BY course_fullname ASC ";
							...
							<input type="hidden" name="course_type_val" id="course_type_val" /> 
							<br />
							
							<label><strong>Internship Hours </strong> <font color="red" style="font-weight: bold;">*</font> </label>  
							<input type="number" name="int_hours" id="int_hours" class="form-control" size="25" min="1" required />  
							
							<input type="hidden" name="int_hours_id" id="int_hours_id" /> 
						</div>  
	...
	$(document).ready(function(){
		...
		function fetch_data(filter_role = ''){
			var dataTable = $('#user_data').DataTable({
				...
				"searching" : true,
				"ajax" : {
					url:"../components/intp-hours/fetch.php",
					type:"POST",
					data:{
		...
		$(document).on('click', '.edit', function(){  
			...
			$.ajax({  
                url:"../components/intp-hours/write.php",  
                method:"POST",  
                data:{id:id},  
                dataType:"json",  
                success:function(data){  
					$('#int_hours_id').val(data.internship_hrs_id); 
                    $('#int_hours').val(data.internship_hrs); 
					
					$('#college_type').val(data.college_id);
					
					$('#course_type').val(data.course_id);					
					$('#course_type_val').val(data.course_id);		
		...
		$('#insert_form').on("submit", function(event){  
			...
			if($('#int_hours').val() == ""){  
				...
				var int_hours = $("#int_hours").val();
				var int_hours_id = $("#int_hours_id").val();
				var course_type = $("#course_type").val();
		
				$.ajax({
					url : "../components/intp-hours/check.php",
					type : "POST",
					cache:false,
					data : {int_hours:int_hours, int_hours_id:int_hours_id, course_type:course_type},
					success:function(result){
						if (result == 1) {
							...
							$.ajax({  
								url:"../components/intp-hours/insert.php",  
								method:"POST",  
		...
		$(document).on('click', '.delete', function(){
			...
			if(confirm("Are you sure you want to remove this?")){
				$.ajax({
					url:"../components/intp-hours/delete.php",
					method:"POST",
					data:{id:id},
					success:function(data){
